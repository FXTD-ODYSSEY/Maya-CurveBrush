...
def forceHistory(cmd, *args, **kwargs):
    if not str(type(cmd)) == "<type 'function'>":
        return
...
def setRebuildCurveStepWnd():
    with window(
        t="set step for rebuilding curve", mnb=0, mxb=0, s=0, rtf=1, w=400, h=30
    ) as wnd:
        with columnLayout():
            with rowLayout(nc=2):
                ...
                slider = floatSliderGrp(
                    l="Width", field=True, min=0, max=100, value=val, pre=2
                )
...
def filletCurves(curves, circ=1, tol=0.001):
    ...
    for c1, c2 in pairs:
        ...
        if (s1 - s2).length() < tol:
            ...
            nodes += filletCurve(
                c1.u[par1], c2.u[par2], ch=1, rpo=1, t=1, jn=0, cir=circ, r=rad, bc=0
            )
    ...
    list(map(lambda n: n.dispCV.set(0), curves))
    select(filletNodes)
    setToolTo("ShowManips")
...
def extedCurvesBidirectional(curves):
    ...
    for crv in curves:
        if "->" in crv.name():
            curve, node = forceHistoryCos(
                extendCurve, crv, cos=1, ch=1, em=0, et=2, d=0.1, s=2, rmk=1
            )
        else:
            curve, node = forceHistory(
                extendCurve, crv, cos=0, ch=1, em=0, et=0, d=0.1, s=2, rmk=1
            )
...
def offsetCurves(curves):
    ...
    for crv in curves:
        ...
        if "->" in crv.name():
            res = offsetCurveOnSurface(
                crv, ch=1, rn=0, cb=2, st=1, cl=1, d=dist, tol=0.0001, sd=5
            )
            ...
            expression(
                s="{0}.cutRadius={0}.distance*0.2".format(res[1].name()),
                o=res[0],
                ae=1,
                uc="all",
            )
...
def smoothCrvOrCVs():
    ...
    for cvs in crvCvs:
        for i in range(iterations + 1):
            ...
            for cv, pos in zip(cvs, pnts):
                move(cv, pos, a=1, ws=1)
...
def detachWHistory(crvPoints):
    ...
    try:
        resPar = crvSh.getParent().getParent()
    except:
        resPar = None
    ...
    for _out in outs:
        if "hyperLayout" in _out[1].node().name():
            continue
        if resCrv == _out[1].node():
            continue
        if detachNode == _out[1].node():
            continue
