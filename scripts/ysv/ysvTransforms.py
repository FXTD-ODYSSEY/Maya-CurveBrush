...
import ysvCurvesOps as yCrv
import ysvPolyOps as yPoly
import ysvUtils
...
def tngConstraint(objs=None, crv=None):
    if objs == None:
        objs = [s for s in ls(sl=1, dag=1, et=nt.Transform) if not type(s.getShape()) == nt.NurbsCurve]
    if crv == None:
        crv = ls(sl=1, dag=1, et=nt.NurbsCurve)
        if crv:crv = crv[0].getParent()
    if objs == None or crv == None:return
...
def duplicateWithConstraints(obj, makeInst=0):
    cons = [node for node in listConnections(obj, s=1, d=0) if isinstance(node, nt.Constraint)]
    if not cons:
    ...
    for constr in set(cons):
        ...
        for p1, p2 in outConns:
            ...
            if source.isConnectedTo(dest):continue
            if dest.isConnectedTo(source):continue
            if isinstance(dest.node(), nt.HyperLayout):continue
...
def dimension(deleteAll=0):
    ...
    sets = [objSet for objSet in  ls(et=nt.ObjectSet) if not objSet.name() == 'defaultLightSet' and not objSet.name() == 'defaultObjectSet']
    for objSet in sets:
...
def placeAlongCurveConnect(obj, crvSh, paramStartDim, paramEndDim, pos):
    ...
    connectAttr(obj.attr('transMinusRotatePivotX'), addDoubleLinear22.attr('input1'), f=1)
    connectAttr(obj.attr('transMinusRotatePivotY'), addDoubleLinear23.attr('input1'), f=1)
    connectAttr(obj.attr('transMinusRotatePivotZ'), addDoubleLinear24.attr('input1'), f=1)
    ...
    connectAttr(paramStartDim.attr('ysvDupAlongCount_minus1'), paramStep.attr('input2X'), f=1)

    connectAttr(paramStep_x_currCount.attr('outputX'), currParamPos.attr('input1D[1]'), f=1)
...
def placeAlongCurve(multiSource=False, seq=True, orderMatters=False):
    ...
    if crvSh:crvSh = crvSh[-1]
    
    if orderMatters:
        sourceObjects = [tr for tr in ls(sl=1, dag=1, et=nt.Transform) if not tr.getShape()==crvSh ]
    else:
        sourceObjects = [tr for tr in ls(sl=1, dag=1, et=nt.Transform) if type(tr.getShape()) == nt.Mesh]
...
def placeAlongCurves(multiSource=False, seq=True):
    ...
    for crvSh in crvs:   
        ...
        paramDims = [sh.getParent() for sh in ls(listFuture(crvSh), et=nt.ParamDimension)]
        delete(paramDims)
...
def disconnectFromCurve():
    ...
    for crvSh in crvs:
        crv = crvSh.getParent()
        fut = listFuture(crv)
        depsTypes = [nt.AddDoubleLinear, nt.MotionPath, nt.MultiplyDivide, nt.PlusMinusAverage]
...
def distributeAlongCurve():
    objs = [s for s in ls(sl=1, dag=1, et=nt.Transform) if not isinstance(s.getShape(), nt.NurbsCurve) and not isinstance(s, nt.Constraint)]
    if not objs:return
...
def motionPathInversAttr(attrName):
    ...
    motionNodes = set([hNode for tr in trs for hNode in set(ls(listConnections(tr), et=nt.MotionPath))])
...
def dupAlongCurvesWnd():
    ...
    with window('ysvDupAlongCrvWnd', t='duplicate Along curves (front -Z, side X)', mnb=0, mxb=0, rtf=1, w=300, h=30) as wnd:
        with columnLayout():
            with rowLayout(nc=7):
                ...
                slider = floatSliderButtonGrp(label='Step: ', bl='multiCurves', bc=Callback(placeAlongCurves), f=True, cw4=(40, 60, 149, 3), w=250, min=0.5, max=100, fmx=100000, v=step, pre=2)
                
                pMenu = popupMenu(p=slider)
                menuItem(p=pMenu, l='MultiSourceSeq', c=Callback(placeAlongCurves, True, True))
                menuItem(p=pMenu, l='MultiSourceRand', c=Callback(placeAlongCurves, True, False))
                menuItem(p=pMenu, l='Select tangentConstraints', c='select(ls(sl=1, dag=1, et=nt.TangentConstraint))')
                menuItem(p=pMenu, l='Revert tangent Axis', c=Callback(revertConstrAxis))
                # button(l="Distribute", c=Callback(distributeAlongCurve))
                
                
                slider1 = floatSliderButtonGrp(label='Count : ', f=True, bl='PlaceAlong', cw=(1, 40), w=250, min=0, max=50, fmx=5000, v=count, pre=0)
                floatSliderButtonGrp(slider1, e=1, cc=Callback(setDistrOptVar, slider1), bc=Callback(placeAlongCurve))
                pMenu = popupMenu(p=slider1)
                menuItem(p=pMenu, l='MultiSourceSeq', c=Callback(placeAlongCurve, True, True))
                menuItem(p=pMenu, l='MultiSourceRand', c=Callback(placeAlongCurve, True, False))
                menuItem(p=pMenu, l='Clear selected curves', c=Callback(disconnectFromCurve))
                menuItem(p=pMenu, l='Invers inverseUp', c=Callback(motionPathInversAttr, 'inverseUp'))
                menuItem(p=pMenu, l='Flip aroundY', c=Callback(flipAroundY))
                
                
                button(l='EdgeLoop2Crv', c=Callback(wrapVertLoop))

                floatSliderButtonGrp(slider, e=1, cc=Callback(calcCountFromStep, slider, slider1))
...
def mirrorScale(obj, axis, direct, keepPivot):
    ...
    pivots = [  [(xMin, yC, zC), (xMax, yC, zC)], [(xC, yMin, zC), (xC, yMax, zC)], [(xC, yC, zMin), (xC, yC, zMax)]  ]
...
def manipHandle2Component() :
    ...
    if type(selComp)==MeshFace:	normal = selComp.getNormal('world')
    elif type(selComp)==MeshEdge: normal = selComp.getPoint(1, 'world') - selComp.getPoint(0, 'world')
    elif type(selComp)==MeshVertex: normal = selComp.getNormal('world')
