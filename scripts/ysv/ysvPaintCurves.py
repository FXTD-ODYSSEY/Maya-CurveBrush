...
class UI():
    ...
    def create(self):
        ...
        elif not isinstance(optionVar(q='ysvPaintCurveShowCVs'), basestring):
            optionVar(sv=('ysvPaintCurveShowCVs', 'No'))
        ...
        with window(self.winName, title=self.winTitle):
            with columnLayout():
                with rowLayout(nc=5):
                    ...
                    text(l = 'Slider min:')
                    self.sliderMinFloatField = floatField(v=sliderMinValue, cc=Callback(self.setMinSlider))
                    
                    stepVal = optionVar(q='ysvPaintCurveStep')
                    self.stepSlider = floatSliderGrp(label='Step : ', field=1, columnWidth=(1, 40), min=sliderMinValue, max=sliderMaxValue, value=stepVal, pre=2, cc=Callback(self.setStep))
                    
                    text(l = 'Slider max:')
                    self.sliderMaxFloatField = floatField(v=sliderMaxValue, cc=Callback(self.setMaxSlider))
                
                
                selectionBehavior = optionVar(q=('ysvPaintCurveSelectBehavior'))
                self.behCheckBox  = checkBox(l='Tweak all(or selected only)', v=selectionBehavior, cc=Callback(self.setSelectBehavior))
                ...
                if showCVs=='Yes':
                    ...
                    checkBoxEn = 0
                self.showCVoption = checkBox(l='Leave cvs visible', v=checkBoxEn, cc=Callback(self.setCVvisibility))
                
                checkBoxEn = optionVar(q=('ysvLockEndCVs'))
                self.lockEndsOption = checkBox(l='Lock ends', v=checkBoxEn, cc=Callback(self.setLockEnds))
                
                checkBoxEn = optionVar(q=('ysvSnapToCurves'))
                self.snapToCurvesOption = checkBox(l='Snap to curves', v=checkBoxEn, cc=Callback(self.setSnapToCurves))
                
                checkBoxEn = optionVar(q=('ysvSnapToMeshes'))
                self.snapToMeshesOption = checkBox(l='Snap to meshes', v=checkBoxEn, cc=Callback(self.setSnapToMeshes))
                
                checkBoxEn = optionVar(q=('ysvCutCurves'))
                self.cutCurvesOption = checkBox(l='Cut curves while snapping', v=checkBoxEn, cc=Callback(self.setCutCurves))

                checkBoxEn = optionVar(q=('ysvSnapToEnds'))
                self.snapToEndsOption = checkBox(l='Snap to ends', v=checkBoxEn, cc=Callback(self.setSnapToEnds))
...
class paintCtx(ysvCtx.baseDraggerCtx):
    ...
    def paintOnRelease(self):
        ...
        sMeshHit = ysvView.closestHitToMeshes(self.meshFns, self.startScreenWSPos, sCVPos - self.startScreenWSPos)
        eMeshHit = ysvView.closestHitToMeshes(self.meshFns, self.endScreenWSPos, eCVPos - self.endScreenWSPos)
    ...
    def paintOnPolyOnPress(self):
        ...
        meshHit = ysvView.closestHitToMeshes(self.meshFns, self.cursorWPos, self.cursorWDir)
    ...
    def getClosestPointToCurves(self, pnt , curves):    
        ...
        for crv in curves:
            ...
            if optionVar(q='ysvSnapToEnds'): tol = 0.4 * crv.__apimfn__().length()
        ...
        if optionVar(q='ysvSnapToEnds'): tolerance = 0.4*abs(ePar-sPar)
        #sPar, ePar = closestCurve.
        
        dPtr = om.MScriptUtil().asDoublePtr()
    
        print ' ' 
        curveFn.closestPoint(om.MPoint(pnt[0], pnt[1], pnt[2]), dPtr, 0.01, om.MSpace.kWorld)
        print ' ' 
    ...
    def snapToCurves(self, cv):
        ...
        try:
            ...
            if curveParamPnt==sPar or curveParamPnt==ePar: return
            ...
            for crv in ls(cuttedCurves, dag=1, et=nt.NurbsCurve, ni=1): crv.dispCV.set(1)            
    ...
    def moveOpOnPress(self, option='singlePoint'):
        ...
        if option == 'softSelectionGlobal' or option=='adjustRadius':
            ...
            for shape in protectObjs:
                if not shape.getShape().type()=='nurbsCurve':
                    ...
                    elif shape.overrideEnabled.get() and not shape.ovverrideDisplayType.get():
                        self.protectShapes.append(shape)
                        shape.overrideEnabled.set(1)
                        shape.overrideDisplayType.set(1)
        ...
        self.startPlaneProjPnt = ysvMath.linePlaneIntersect(l0, l1,  self.midCvWPos, self.cursorWDir)
    ...
    def moveOpOnDrag(self, option=''): 
        ...
        dragPlaneProjPnt = ysvMath.linePlaneIntersect(l0, l1,   self.midCvWPos, self.cursorWDir)
