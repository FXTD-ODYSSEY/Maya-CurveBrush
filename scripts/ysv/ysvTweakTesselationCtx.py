...
class ctx(baseDraggerCtx):
    def __init__(self, ctxName):
        ...
        try: self.currentMesh = ls(sl=1, dag=1, et=nt.Mesh, ni=1)[0]
        except: self.currentMesh = None
    ...
    def selectPoly(self):
        ...
        self.currentMesh = ysvView.closestMeshFromHit(meshes, self.cursorWPos, self.cursorWDir)
    ...
    def getClosestBoundaryInCurve(self):
        try: 
            ...
            try:
                self.meshHit = ysvView.closestHitToMeshes([self.currentMesh.__apimfn__()], self.cursorWPos, self.cursorWDir)
            except:
                self.meshHit = None
                return
            ...
            if len(inCurves)==4:
                if self.closestCurve==self.inCurve1 or self.closestCurve==self.inCurve3:
                    self.direction = 1
                elif self.closestCurve==self.inCurve2 or self.closestCurve==self.inCurve4:
                    self.direction = 2
            else:
                if self.closestCurve==self.inCurve1 or self.closestCurve==self.inCurve2:
                    self.direction = 1
...
#-------------------------------------MMB-----------------------------------
    ...
    def MMBOnDrag(self, mult=0.02):
        ...
        try: inViewMessage(msg=int(value), pos = 'midCenterTop', dk=1)
        except: pass
